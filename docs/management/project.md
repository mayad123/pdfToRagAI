...
2. **“Try it in 60 seconds” web UI** — Multi-page **vanilla HTML** under `public/` (no framework build): use-focused home, setup, about, and **`/demo.html`** where you point at an **allowlisted corpus directory**, run **Ingest**, then **Query** to see **passages with file + page citations**. The demo loads the MCP client from a CDN and uses Streamable HTTP to **`/mcp`** on the same host (**F19**). Turns a CLI-first tool into something demoable in a browser after **`npm run mcp:http`**.
