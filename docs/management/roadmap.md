...
| Interactive web demo: vanilla HTML over HTTP/SSE; select/upload PDF, ingest, query, cited passages (highlighting desirable) | **F19** | Done | `public/index.html`, `src/mcp/server-http.ts` (static serving) |
...
| 4 | **F19** | **`pdf-to-rag-mcp-http`**; **`PDF_TO_RAG_*`** allowlist; browser; **`GET /`** serves [public/index.html](../../public/index.html) (see [server-http.ts](../../src/mcp/server-http.ts)) | Single-page **vanilla** UI: corpus path (absolute, allowlisted), **Ingest**, **Query**, results with **file / page / excerpt**; browser loads MCP client from **esm.sh** (or equivalent) and uses **Streamable HTTP** to `POST /mcp` | `public/index.html` + operator notes in [use/mcp.md](../use/mcp.md) | Localhost end-to-end works with documented env |
