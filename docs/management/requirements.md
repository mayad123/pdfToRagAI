...
| **What ships** | Paths in `package.json` **`files`**: `dist`, `LICENSE`, `README.md`, `docs`, `public` (vanilla web demo for HTTP MCP). Not shipped: `src/`, `.cursor/`, `.hooks/`, `scripts/` (except what npm includes by default—none of these are in `files`). |
...
| F19 | **Browser-friendly demo (optional):** A minimal, no-build **vanilla HTML** client (e.g. served with the HTTP MCP server) that can **select or upload a PDF**, trigger **ingest**, submit a **question**, and show **cited passages** (file + page, excerpt text — highlighting in UI is desirable). Uses **`fetch`** (or equivalent) against the HTTP/SSE MCP endpoint; no frontend framework or extra npm dependencies in the demo artifact. |
